...
Anml_VarIndex:
	...
	; 2: Penguin
	; 3: Seal
	; 4: Pig, Bear (all three)
	; 5: Flicky (both), Dove
	; 6: Squirrel, Mouse
	; Do these according to the PLCs you gave.
	dc.b 4,	5	; BGZ
	dc.b 2, 3	; SRZ
	dc.b 6, 3	; MTZ
	dc.b 4, 5	; HMZ
	...
	dc.b 6, 3	; HPZ
	dc.b 0, 5	; TMZ
	zonewarning	Anml_VarIndex,(1*2)

Anml_Variables:
	; Format:
	; dc.w horizontal speed, vertical speed
	; dc.l mappings address
		...
		dc.w $FD80, $FC80
		dc.l Map_Animal3
	zonewarning	Anml_Variables,(2*4)	; Two words and one longword.
...
Anml_Ending:	; Routine 0
		...
		move.w	(a1,d0.w),Anml_VelX(a0) ; load horizontal speed
		move.w	(a1,d0.w),obVelX(a0)
		move.w	2(a1,d0.w),Anml_VelY(a0) ; load vertical speed
...
Anml_FromEnemy:
		...
		adda.w	d0,a1
		move.w	(a1)+,Anml_VelX(a0)	; load horizontal speed
		move.w	(a1)+,Anml_VelY(a0)	; load vertical	speed
		move.l	(a1)+,obMap(a0)	; load mappings
		move.w	#vramAnimal1,obGfx(a0)	; VRAM setting for 1st animal
		btst	#0,$30(a0)	; is 1st animal	used?
		beq.s	loc_90C0	; if yes, branch
		move.w	#vramAnimal2,obGfx(a0)	; VRAM setting for 2nd animal
...
loc_912A:
		...
		bpl.s	loc_9180
		add.w	d1,obY(a0)
		move.w	Anml_VelX(a0),obVelX(a0)
		move.w	Anml_VelY(a0),obVelY(a0)
		move.b	#1,obFrame(a0)
		move.b	$30(a0),d0
		add.b	d0,d0
		addq.b	#4,d0
