...
@islevel:
		moveq	#id_EndZ+1,d0	; actually ends up being @LBGZ because fuck you lole
		cmpi.w	#(id_BGZ<<8)+3,(v_zone).w	; seriously this is bad please don't do this
		beq.s	@selectlist	; god it's awful
	@notlbgz:
		moveq	#0,d0
		move.b	(v_zone).w,d0
...
Debug_Action:	; Routine 2
		...
		moveq	#id_EndZ+1,d0	; actually ends up being @LBGZ because fuck you lole
		cmpi.w	#(id_BGZ<<8)+3,(v_zone).w	; seriously this is bad please don't do this
		beq.s	@isntlevel	; god it's awful
...
@backtonormal:
	...
	; Relocated all of this to make sure d0 was clean.
		move.b	d0,(v_player+obFrame).w
		move.b	d0,(v_player+obAniFrame).w
		move.b	d0,(v_player+obAnim).w
		move.w	d0,obX+2(a0)
		move.w	d0,obY+2(a0)
		move.l	d0,obVelX(a0)	; Took me long enough to actually do this.
		move.w	d0,obInertia(a0)
	; End of moved block.
		bsr.w	Hud_Base
		move.b	#1,(f_scorecount).w
		move.b	#-$80,(f_ringcount).w
		jsr		ReloadPlayerMappings
		move.w	#vramCharacter,(v_player+obGfx).w
	; The above block (indicated with a comment) used to be here.
