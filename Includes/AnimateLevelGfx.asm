...
; ===========================================================================
AniArt_Index:	dc.w AniArt_BGZ-AniArt_Index, AniArt_none-AniArt_Index
		dc.w AniArt_MZ-AniArt_Index, AniArt_none-AniArt_Index
		dc.w AniArt_none-AniArt_Index, AniArt_SBZ-AniArt_Index
        dc.w AniArt_MHZ-AniArt_Index, AniArt_HPZ-AniArt_Index
		dc.w AniArt_none-AniArt_index
		zonewarning AniArt_Index,2
		dc.w AniArt_Ending-AniArt_Index
; ===========================================================================
; ---------------------------------------------------------------------------
; Animated pattern routine - Blossom Gardens
; ---------------------------------------------------------------------------

AniArt_BGZ:
		cmpi.b	#3,(v_act).w
		beq.w	AniArt_GHZ_Waterfall	; Legacy BGZ
		cmpi.b	#1,(v_act).w
		beq.w	AniArt_BGZ_Waterfall
AniArt_BGZ_Seaweed:
@size:		equ 4

		subq.b	#1,(v_lani0_time).w
		bpl.s	AniArt_BGZ_Waterfall
	...
	@isframe1:
		...
		locVRAM	(vramSeaweedBGZ*$20)
		lea	(Art_BGZSeaweed).l,a1 ;	load small flower patterns
		lea	(a1,d0.w),a1	; jump to appropriate tile
		move.w	#@size-1,d1
		bra.w	LoadTiles
		
@sequence:	dc.b 0,	1, 2, 3
		rts

; ---------------------------------------------------------------------------

AniArt_BGZ_Waterfall:		
@size:		equ 8	; number of tiles per frame

		subq.b	#1,(v_lani1_time).w ; decrement timer
		bpl.s	@end ; branch if not 0

		move.b	#3,(v_lani1_time).w ; time to display each frame
		lea	(Art_BGZWater).l,a1 ; load waterfall patterns
		moveq	#0,d0
        move.b    (v_lani1_frame).w,d0
        addq.b    #1,d0
		cmpi.b	#4,d0		; there are 3 frames
		bne.s	@frame01or2	; branch if frame 0, 1 or 2
		moveq	#0,d0
		
		
@frame01or2:
		move.b	d0,(v_lani1_frame).w
		mulu.w	#@size*$20,d0
		adda.w	d0,a1   ; jump to appropriate tile
		locVRAM	(vramWFallBGZ*$20)		; VRAM address
...
; ===========================================================================
; ---------------------------------------------------------------------------
; Animated pattern routine - Melting Heights
; ---------------------------------------------------------------------------
AniArt_MHZ:
...
@size:		equ 8	; number of tiles per frame
		...
		subq.b	#1,(v_lani0_time).w ; decrement timer
		bpl.s	AniArt_MHZ_Magma	; branch if not 0
...
@size:		equ 8	; number of tiles per frame
		subq.b	#1,(v_lani0_time).w
		bpl.s	AniArt_HPZ_OrbObject	; noUpdate
		move.b	#6,(v_lani0_time).w
		moveq	#0,d0
		move.b	(v_lani0_frame).w,d0
		addq.b	#2,(v_lani0_frame).w ; increment frame counter
		cmpi.b	#6*2,(v_lani0_frame).w
		bcs.s	@noWrap
		clr.b	(v_lani0_frame).w

	@noWrap:
        lea       (Art_HPZPulseOrb).l,a2       ; Orb art

        move.w    @offsets1(pc,d0.w),d1        ; Orb 1
        lea       (a2,d1.w),a1
        locVRAM   $5E00
        moveq     #@size-1,d1
        bsr.w     LoadTiles

        move.w    @offsets2(pc,d0.w),d1        ; Orb 2
        lea       (a2,d1.w),a1
        moveq     #@size-1,d1
        bsr.w     LoadTiles

        move.w    @offsets3(pc,d0.w),d1        ; Orb 3
        lea       (a2,d1.w),a1
        moveq     #@size-1,d1
        bsr.w     LoadTiles
		bra.s	AniArt_HPZ_OrbObject
...
AniArt_HPZ_OrbObject:
@size:		equ 8	; number of tiles per frame
		subq.b	#1,(v_lani1_time).w
		bpl.s	@end	; noUpdate
		move.b	#5,(v_lani1_time).w
		moveq	#0,d0
		move.b	(v_lani1_frame).w,d0
		addq.b	#2,(v_lani1_frame).w ; increment frame counter
		cmpi.b	#13*2,(v_lani1_frame).w
		bcs.s	@noWrap
		clr.b	(v_lani1_frame).w

	@noWrap:
		move.w	@offsets(pc,d0.w),d0
		; setup vdp command
		lea	(Art_HPZObjectOrb).l,a1 ;	load small flower patterns
		adda.w	d0,a1		; jump to appropriate tile
		locVRAM	$6C40
		; read tiles
		move.w	#@size-1,d1
