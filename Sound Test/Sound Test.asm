...
DMAto		macro	Size, Source, Destination
...
GM_SoundTest:
		...
		jsr	PaletteFadeOut				; fade the screen out

	; --- Setup/clearing ---

;		disable_ints				; disable interrupts ; Seems unnecessary.
...
ST_ClearMain:
		...
		lea	(v_spritetablebuffer).w,a1			; sprite RAM
...
ST_ClearSprites:
		...
		lea	(v_hscrolltablebuffer).w,a1			; H-Scroll RAM
...
ST_ClearHScroll:
		...
		move.l	d0,(v_scrposy_dup).w			; clear V-scroll

	; --- Loading data ---

		lea	(Pal_Sound).l,a0			; load palette data
		lea	(v_pal_dry_dup).w,a1			; load palette RAM
...
ST_LoadPal:
	...
	DMAto	(Map_Piano-Art_Piano), Art_Piano, $40000000
	DMAto	(Art_End-Art_Extras), Art_Extras, $6C000002
	DMAto	(Art_Font-Art_Keys), Art_Keys, $50000002
	DMAto	(Map_Font-Art_Font), Art_Font, $58000002
...
ML_SoundTest:
	...
	vsync							; wait for V-blank
		...
		btst	#bitB,(v_jpadpress1).w				; has start been pressed?
		beq.s	ML_SoundTest				; if not, branch
		move.l	#VBlank,VBlankRout.w
		move.b	#id_Title,(v_gamemode).w			; set game mode to 04 (Title Screen)
		moveq	#0,d0
		lea		($FFFFA000).w,a1
		move.w	#($FFFFA038-$FFFFA000)/4-1,d1
	@clearram:
		move.l	d0,(a1)+
		dbf		d1,@clearram	; fill object space ($D000-$EFFF) with 0
		jmp	GM_Title
...
ST_ScrollFont:
		...
		lea	(v_hscrolltablebuffer+$2F4).w,a1			; load scroll buffer area where text is
...
STDF_NoNameFinish2:
		...
		enable_ints				; re-enable interrupts
...
STC_TopBar:
		...
		move.b	(v_jpadpress1).w,d5			; load pressed buttons
		btst.l	#$01,d5					; was down pressed?
		beq.s	STC_TB_NoDown				; if not, branch
		andi.w	#%1111110011111100,(v_jpadhold1).w	; clear up/down buttons
...
STC_TB_NoPress:
		move.b	(v_jpadhold1).w,d5			; load held buttons
		andi.b	#%00001100,d5				; get only left/right
...
STC_BB_NoRight:
		move.b	(v_jpadpress1).w,d5			; reload pressed buttons
...
STC_BB_NoPress:
		move.b	(v_jpadhold1).w,d5			; reload pressed buttons
		andi.b	#%01100000,d5				; get A or C
...
STC_Volume:
		...
		subq.b	#$01,(Drvmem+$16).w			; decrease the music volume
		bpl.s	STC_UpdateVolume			; if still valid, branch
		sf.b	(Drvmem+$16).w				; keep volume at 00

STC_UpdateVolume:
		move.b	(Drvmem+$16).w,d0				; load volume to d0
...
STC_V_NoA:
		addq.b	#$01,(Drvmem+$16).w			; increase the music volume
		cmpi.b	#$7F,(Drvmem+$16).w			; is it at maximum volume?
		bls.s	STC_UpdateVolume			; if not, branch
		move.b	#$7F,(Drvmem+$16).w			; force to maximum volume
...
STC_FM1:
		not.b	(Drvmem+$A0+$21).w
		rts

STC_FM2:
		not.b	(Drvmem+$D0+$21).w
		rts

STC_FM3:
		not.b	(Drvmem+$100+$21).w
		rts

STC_FM4:
		not.b	(Drvmem+$130+$21).w
		rts

STC_FM5:
		not.b	(Drvmem+$160+$21).w
		rts

STC_FM6:
		not.b	(Drvmem+$190+$21).w
		rts

STC_PCM1:
		not.b	(Drvmem+$40+$21).w
		rts

STC_PCM2:
		not.b	(Drvmem+070+$21).w
		rts

STC_PSG1:
		not.b	(Drvmem+$1C0+$21).w
		rts

STC_PSG2:
		not.b	(Drvmem+$1F0+$21).w
		rts

STC_PSG3:
		not.b	(Drvmem+$220+$21).w
		rts
...
ST_DrawKeys:
		...
		lea	(v_spritetablebuffer).w,a1			; load sprite RAM
...
STDK_ChanRouts:	dc.l	v_pal_dry+$22, STDK_ChanFM
		dc.l	v_pal_dry+$26, STDK_ChanFM
		dc.l	v_pal_dry+$2A, STDK_ChanFM
		dc.l	v_pal_dry+$2E, STDK_ChanFM
		dc.l	v_pal_dry+$32, STDK_ChanFM
		dc.l	v_pal_dry+$42, STDK_ChanPCM
		dc.l	v_pal_dry+$46, STDK_ChanPCM
		dc.l	v_pal_dry+$4A, STDK_ChanPSG
		dc.l	v_pal_dry+$4E, STDK_ChanPSG
		dc.l	v_pal_dry+$52, STDK_ChanPSG
		dc.l	v_pal_dry+$36, STDK_ChanFM
...
HB_SoundTest:
		move.w	#$8A00|$00,(vdp_control_port).l			; set new interrupt line occurance amount
...
HBST_FontPos:	move.l	($FFFF5E00).l,(vdp_data_port).l		; change V-scroll position
		...
		move.w	#$8A00|$DF,(vdp_control_port).l			; revert align amount
		move.l	#NullBlank,(HBlankRout).w		; set H-blank routine
...
VB_SoundTest:
		...
		move.w	#$8A00|($B7/$02),(vdp_control_port).l		; set interrupt line address
		move.l	#HB_SoundTest,(HBlankRout).w		; set H-blank routine

VB_SoundTest_NoHB:
		movem.l	d0-a6,-(sp)				; store register data
		tst.b	(v_vbla_routine).w				; was the 68k late?
		...
		DMAto	$0B00, $FFFF8000, $60800003		; background plane

VBST_NoDrawPlane:
		DMAto	$0080, v_pal_dry, $C0000000		; palette
		DMAto	$0280, v_spritetablebuffer, $78000003		; sprites
		DMAto	$0380, v_hscrolltablebuffer, $7C000003		; h-scroll
		move.l	#$40000010,(a6)				; v-scroll
		move.l	(v_scrposy_dup).w,(a5)			; ''
		jsr	ReadJoypads				; read the controller pads

VBST_68kLate:
		sf.b	(v_vbla_routine).w				; clear V-blank flag
...
Pal_Sound:	incbin	"Sound Test/Data/Pal Piano.bin"
		dc.w	$0000,$00EE,$0048,$00EE,$0048,$00EE,$0048,$00EE
		dc.w	$0048,$00EE,$0048,$00EE,$0048,$0008,$0004,$0000
		dc.w	$0000,$0E00,$0004,$0E00,$0004,$08EE,$0080,$08EE
		dc.w	$0080,$08EE,$0080,$0000,$0000,$0000,$0000,$0000
		dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
		dc.w	$0000,$0000,$0000,$0000,$0000,$0000,$0000,$0000
Pal_Sound_End:	even

Art_Piano:	incbin	"Sound Test/Data/Art Piano.unc"
		even
Map_Piano:	incbin	"Sound Test/Data/Map Piano.unc"
		even

Art_Keys:	incbin	"Sound Test/Data/Art Keys.unc"
		even

Art_Font:	incbin	"Sound Test/Data/Art Font.unc"
		even
Map_Font:	incbin	"Sound Test/Data/Map Font.bin"
		even

Art_Extras:	incbin	"Sound Test/Data/Art Extras.unc"
		even
