...
namespace SHIMAObjectDefinitions.LBGZ
{
	class SwingingPlatform : ObjectDefinition
	{
		...
		{
			imgwreckingball = ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../art/nemesis/GHZ Giant Ball.bin", CompressionType.Nemesis), "../Mappings/GHZ Ball.asm", 1, 2);
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../art/nemesis/GHZ Swinging Platform.bin", CompressionType.Nemesis), "../Mappings/Swinging Platforms (GHZ).asm", labels[i], i == 1 ? 0 : 2));
		}
